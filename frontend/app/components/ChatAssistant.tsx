...
export default function ChatAssistant({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "ai",
      content: "Hi! I'm your AI Knowledge Worker. How can I help you today?",
    },
  ]);
  ...
  const formatMessage = (text: string) => {
    ...
    return lines.map((line, i) => {
      ...
      return (
        <div key={i} className={isBullet ? "" : "mb-1 min-h-[1em]"}>
          {content}
        </div>
      );
    });
  ...
  const parseInlineStyles = (text: string): React.ReactNode[] => {
    // Matches **bold**, `code`, and [text](url)
    const regex = /(\*\*.*?\*\*|`.*?`|\[.*?\]\(.*?\))/g;
    ...
    return parts.map((part, index) => {
      ...
      if (part.startsWith("`") && part.endsWith("`")) {
        return (
          <code
            key={index}
            className="bg-white/15 px-1 py-0.5 rounded text-purple-300 font-mono text-xs"
          >
            {part.slice(1, -1)}
          </code>
        );
  ...
  const sendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", content: "Sorry, I am currently offline." },
      ]);
    } finally {
      setLoading(false);
    }
      ...
      {isOpen && (
        <div
          style={{
            ...
            borderRadius: "20px",
            boxShadow:
              "0 24px 64px rgba(0,0,0,0.6), 0 0 40px rgba(168,85,247,0.15)",
        ...
        >
          ...
          <div
            style={{
              ...
              background:
                "linear-gradient(to right, rgba(168,85,247,0.1), rgba(18,18,24,0))",
            }}
          >
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  color:
                    input.trim() && !loading ? "#fff" : "rgba(255,255,255,0.4)",
