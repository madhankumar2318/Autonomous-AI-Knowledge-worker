  ...
  return (
    <div className="space-y-3">
      ...
      {articles.length > 0 ? (
        ...
        <div className="flex flex-col items-center top-[10%] relative justify-center py-16 text-center animate-fade-in" style={{ background: "rgba(255,255,255,0.02)", borderRadius: "16px", border: "1px dashed rgba(255,255,255,0.1)" }}>
          <div className="w-16 h-16 rounded-2xl mb-4 flex items-center justify-center shadow-lg" style={{ background: "linear-gradient(135deg, rgba(168,85,247,0.1), rgba(99,102,241,0.1))", border: "1px solid rgba(168,85,247,0.2)" }}>
            <SearchX className="w-8 h-8 text-primary" style={{ color: "#a855f7" }} />
          </div>
          <h3 className="text-lg font-bold text-white mb-2 tracking-tight">No articles found</h3>
          <p className="text-sm text-secondary max-w-sm px-4 leading-relaxed mb-6">
            We couldn't find any news matching <strong className="text-primary tracking-wide">"{topic}"</strong> {category && `in ${category}`}.
            <br />Try refining your search or relaxing the filters.
          </p>
          <button
            type="button"
            onClick={() => { setTopic(""); setCategory(""); }}
